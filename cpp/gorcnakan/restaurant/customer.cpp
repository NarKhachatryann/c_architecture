...
Customer::Customer(Customer&& other) noexcept : name(std::move(other.name)),
 contactInfo(std::move(other.contactInfo)), orderHistory(std::move(other.orderHistory)) {}
...
void Customer::viewOrderHistory() const {
    for (int i = 0; i < orderHistory.size(); i++) {
        std::string order = orderHistory[i].getOrder();
    }
}
